...
- Tại màn hình đăng nhập `LoginScreen`: Có thêm nút liên kết *"Khám phá ngay (Không cần đăng nhập)"* hoặc *"Sử dụng nhận diện nấm"* dẫn thẳng đến `HomeScreen` với tab Nhận diện / Từ điển.
...
## UI composition
`Scaffold > SafeArea > Center > SingleChildScrollView > Column(stretch)`: icon, title text, `CustomTextField` (username), `CustomTextField` (password, `obscure: _obscure`, eye-icon `suffixIcon` toggles it), conditional error `Text`, `CustomButton` (`loading: _loading`), và nút TextButton *"Sử dụng tính năng Nhận diện Nấm không cần đăng nhập"*.
