...
`FacilityListScreen` được mở từ mục thứ 3 trong sidebar của `HomeScreen`.

Màn hình chỉ được hiển thị khi `LocalSession.isLoggedIn == true`.
Nếu chưa đăng nhập, Home hiển thị màn hình yêu cầu đăng nhập.
